"""``audio`` command-line interface: a thin shell over the public API.

    audio diagnose clip.wav            # full model: verdict + knock + causes
    audio triage   clip.wav            # calibrated engine-vs-running-gear
    audio clean    clip.wav            # isolate mechanical sound (no model)
    audio serve                        # local web upload app
    audio scrape   youtube|tiktok      # (reddit deprecated, too noisy)
    audio train

Every command imports its heavy dependencies lazily, so `audio --help` and the
...
app = typer.Typer(
    ...
    help="Diagnose a car's mechanical fault from the sound it makes.\n\n"
         "New here? Run [bold]audio doctor[/bold] — it checks your setup and "
...
def diagnose(
    ...
    if as_json:
        ...
        _nudge(f"see *why* (spectrograms + scores):  audio inspect {audio} -o report.html",
               f"coarse calibrated call:  audio triage {audio}")
...
def triage(
    ...
    print("  " + "─" * 56 + "\n")
    _nudge(f"full diagnosis with causes:  audio diagnose {audio}")
...
def clean(
    ...
    _nudge(f"diagnose what was isolated:  audio diagnose {audio}",
           f"see it visually:  audio inspect {audio} -o report.html")
...
def serve(
    ...
    data/training/ from your own `audio train` (or none)."""
    import os
    ...
    if host not in ("127.0.0.1", "localhost", "::1"):
        ...
        Console(stderr=True).print(
            f"[bold red]⚠ exposing audio on {host} with NO authentication[/bold red] "
            f"— anyone who can reach this host can upload files and run inference.")
    try:
        import uvicorn
    except ImportError:
        raise typer.Exit(code=_missing("audio serve", "web")) from None
...
def scrape(
    ...
    data/<platform>/corpus.jsonl ready for `audio train` (no LLM, no external
    ...
    _nudge("audit what you collected:  audio gallery -o gallery.html",
           "scrape the other sources too, then:  audio train")
...
def ingest(
    ...
    spans) and add them to the corpus, ready for `audio train`. One coherent
    ...
    _nudge("see what was isolated:  audio gallery -o gallery.html",
           "train on it:  audio train")
...
def train(
    ...
    if fixtures:
        ...
        _nudge("now diagnose a clip:  audio diagnose <clip.wav>",
               "ready for a real model? scrape:  audio scrape youtube")
    else:
        build.train(min_class=min_class, prune_noisy=prune_noisy)
        _nudge("diagnose a clip:  audio diagnose <clip.wav>",
               "audit the corpus:  audio gallery -o gallery.html")
...
def start():
    ...
    from cardiag.pipeline import build
    c = Console()
    c.print("\n[bold]Welcome to audio.[/bold] Getting you set up…\n")
    c.print("[bold]1/2[/bold]  environment check")
    if doc.run():
        c.print("[yellow]Fix the ✗ items above, then run [bold]audio start[/bold] "
                "again.[/yellow]")
        raise typer.Exit(1)
    ...
    c.print(f"  [bold]audio diagnose {paths.DEMO_CLIP.name}[/bold]   (or any clip of your own)")
    c.print("  [bold]audio serve --model models[/bold]   the live web app — paste a link or "
            "drop a clip")
    c.print("  [bold]audio demo[/bold]                   the whole loop, scraping for real\n")
...
def demo(
    ...
    _nudge("see/hear what happened:  audio inspect --sample 6 -o report.html",
           "browse your corpus:  audio gallery -o gallery.html",
           "scale up for a better model:  audio scrape youtube --per-query 5 --max-videos 200")
